...
def test_parser_exposes_required_commands() -> None:
    ...
    parser = build_parser()
    init_args = [
        "init", "--coordination-root", "C:/coord",
        "--backend-root", "C:/backend", "--mobile-root", "C:/mobile",
        "--frozen-worktree", "C:/frozen",
    ]
    ...
    parsed = parser.parse_args(["confirm-resets", "--coordination-root", "C:/coord", "--remaining", "2"])
...
def test_active_cycle_uses_injected_invoker(tmp_path: Path) -> None:
    ...
    frozen = tmp_path / "rex-ai-pc-test"; frozen.mkdir()
    root.mkdir()
    config = replace(initialize_runtime(root, backend, mobile, frozen), observe_only=False)
    class Invoker:
        def implement(self, *args): return AgentResult("continue", "ok", "continue")
        def review(self, *args): raise AssertionError("unexpected review")
        def lead(self, *args, **kwargs): return AgentResult("done", "none", "")
