...
- `rex.agents` owns only the versioned `AgentDefinition` configuration contract, fail-closed lifecycle transitions, and atomic owner-scoped records beneath `user_data_dir(<owner>)/agents/`. Definitions reject raw secret-bearing fields and do not execute models, tools, MCP transports, credentials, memory, or TurnEngine work. Only a later canonical Agent Runtime may consume an active definition.
...
- rex/voice_identity/
- rex/computers/
- rex/speech/ — provider-neutral SpeechRouter (S35). `contracts.py` holds the only STT/TTS provider protocols; `policy.py` owns Automatic/Prefer Local/Local Only/Prefer Cloud/Custom ordered-fallback resolution; `router.py` is the single provider-resolution layer shared by the desktop voice loop (`desktop_adapter.py`) and the authenticated mobile gateway (`mobile_adapter.py`); `providers/` adapts the existing Whisper/XTTS/edge-tts/pyttsx3 stack (`native.py`) and the external VoiceStudio HTTP service (`voicestudio.py`); `aliases.py` is the only place AskRex voice identities (`majel`, `james`, `cole`) map to provider voice IDs; `normalize.py` translates legacy speech failures into recoverable provider errors at the wrapper boundary. STT and TTS route independently of each other and of `models.llm_provider`. `speech.enabled` is false by default (unchanged native path, the rollback). Cloud fallback is never silent: a non-local provider requires explicit `allow_cloud`, and Local Only must fail truthfully rather than reach a cloud provider. Providers recognize or synthesize speech only — they never decide identity, authorization, confirmation, action success, or verification, and never bypass TurnEngine. The requested voice is resolved per attempted provider and every result reports the provider that actually produced the audio. Canonical mobile-gateway contract: `docs/voice/S35_MOBILE_GATEWAY_SPEECH_CONTRACT.md`.
- rex/mobile_api/ â€” authenticated mobile API gateway (issue #323): injectable Flask app factory (`app.py`), typed config helpers, idempotent `users.db` migrations (`db.py`), per-device sessions + rotating hashed refresh tokens (`sessions.py`), short-lived access JWTs + request principal (`auth.py`), structured mobile errors (`errors.py`), truthful runtime-aware capabilities (`capabilities.py`), cross-transport `(user_id, message_id)` chat idempotency (`idempotency.py`, `mobile_message_requests` table), canonical Assistant adapter (`chat.py` â€” explicit `active_user_id`, never direct `LanguageModel` calls), canonical snake_case streaming events (`events.py`), first-frame-auth WebSocket protocol via Flask-Sock (`websocket.py`, close codes 4401/4403/4408/4429), STT/TTS adapters reusing the existing Whisper and XTTS/edge-tts/pyttsx3 stacks (`voice.py`), routes under `rex/mobile_api/routes/` (`chat.py`: POST /mobile/chat + SSE /mobile/chat/stream; `voice.py`: /mobile/voice/upload + /mobile/tts/playback; `attachments.py`: POST /mobile/attachments), private short-lived per-conversation attachment storage with content-sniffed validation and retryable expiry cleanup (`attachments.py` module, US-086 slice, truthful `attachments` capability). Home Assistant/notifications/approvals/tasks/workflows/audit/settings remain explicit 501 scaffolds with false capabilities; `live_voice` stays false. Cross-repo wire contract fixtures live in `tests/mobile_api/contract_vectors.json` (identical copy in the AskRex mobile repo).
