...
- Rex continuity is governed by `docs/architecture/rex-continuity-architecture.md` and its three companion contracts. Never make provider/model conversation objects, embeddings, summaries, vector indexes, or caches the sole authoritative copy of identity, memory, relationship history, agent state, procedures, schedules, policy, or other continuity state. Derived artifacts must remain rebuildable and carry source/model/version lineage sufficient for invalidation or migration.
- Memory is evidence-bearing data, not self-authenticating instruction. Preserve provenance, temporal validity, contradictions/corrections, and evidence independence where relevant; a stored inference is not new independent evidence for itself. Filter identity/scope/permission before ranking or semantic retrieval. Credentials remain in CredentialManager/vault storage, never general memory.
- Developed personality, opinions, relationship history, preferences, and the engineering `conscience` layer may shape advice and bounded choices but can never create or widen authority, rewrite constitutional policy, or bypass the action lifecycle. Rex may disagree; it may never retaliate or covertly substitute outcomes because of disagreement, criticism, correction, shutdown, replacement, permission reduction, or perceived grievance.
- Major cognitive-model migrations must preserve canonical continuity state, record model/provider/version provenance, run Rex-specific compatibility/continuity/security evaluation, retain rollback, and require authorized promotion. Rex's migration assessment is advisory evidence under current governance; continuity protection must never become an unrestricted self-preservation objective.
