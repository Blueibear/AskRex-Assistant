...
- Claude Code cloud sessions are asynchronous GitHub workers, not synchronous CLI results. On Windows, new cloud sessions require a real TTY, so the orchestrator must launch them through the Windows-only `pywinpty`/ConPTY dev dependency rather than a captured subprocess or transcript workaround. Dispatch only from a clean paused role worktree onto a dedicated pushed `ralph/cloud/...` branch; persist the returned cloud session ID/URL outside the repo, require an exact final `ralph-cloud-complete: <task_id>` commit subject before adoption, and refuse automatic adoption if the local role worktree moved after dispatch. Cloud workers may push only their dedicated branch and never merge or claim physical-device verification.
