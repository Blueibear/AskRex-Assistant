...
- The base install includes `jsonschema>=4.25.1`; orchestrator agent output must pass the canonical Draft 2020-12 schema before any state change, coordination write, or repository publication.
...
- Claude Code cloud sessions are asynchronous GitHub workers, not synchronous CLI results. On Windows, new cloud sessions require a real TTY, so the orchestrator must launch them through the Windows-only `pywinpty`/ConPTY dev dependency rather than a captured subprocess or transcript workaround. Automatic routing may use `fallback` or `preferred` mode, but each task still dispatches only from a clean leased role worktree onto a dedicated pushed `ralph/cloud/...` branch; persist the returned cloud session ID/URL outside the repo, never duplicate an active task session, require an exact final `ralph-cloud-complete: <task_id>` commit subject before adoption, publish the adopted result through the normal durable handoff receipt, and refuse adoption if the local role worktree moved after dispatch. Cloud workers may push only their dedicated branch and never merge or claim physical-device verification.
- Claude cloud workers do not inherit the local coordination directory. Every cloud implementation dispatch must include the same bounded coordination context supplied to the local implementation path. When a persisted cloud session belongs to a different task, evaluate its age before rejecting the current task: an expired mismatched record is stale and must be cleared so the current task can dispatch, while a still-live mismatched session remains a hard conflict and must not be replaced.
- Coordination-only/evidence-closeout tasks must be recognized from stable task-id markers plus punctuation-tolerant no-product-mutation wording. Do not run the full product validation suite merely because a read-only closeout says `do not modify source, tests` instead of the slash-form phrase `do not modify source/tests`.
...
- On Windows, orchestrator Codex CLI invocations use Codex's `windows.sandbox="elevated"` backend because the unelevated backend has reproduced intermittent child-process initialization failures (`0xC0000142`). This does not widen Rex's write authority: implementation still runs only with Codex `workspace-write` inside the disposable scratch clone, review/lead remain read-only, Git metadata stays hidden from implementation, and out-of-workspace writes must remain denied. Do not change this sandbox backend or its containment controls without explicit owner approval and a boundary regression.
- Treat Codex Windows `connecting runner pipe-in` / `Failed to create unified exec process` failures as transient infrastructure faults whether they arrive as a nonzero CLI failure or a structured agent blocker. Reset stale `codex-command-runner.exe` helpers before retrying. When the packaged `CodexSandboxService.OpenAI.Codex` Windows service is installed, recovery must restart that SCM-managed service and confirm it reaches Running before the next retry; do not treat the service executable as an ordinary kill-only helper. Use standalone `codex-windows-sandbox-service.exe` termination only as a legacy fallback, and allow at least four bounded recovery attempts before parking the workstream.
...
Development may request `fixed-needs-retest`, but orchestrated development must not rewrite canonical `TEST-*.md` issue status. It emits a retest request to the testing mailbox and releases the backend/mobile development role back to idle/planning so unrelated campaign work can continue while testing owns the physical retest. The testing role owns canonical status changes and may mark `verified` only after the real UI/device path is retested. Never place credentials, private conversation content, or secrets in coordination files.

The optional local development orchestrator in `scripts/dev_orchestrator/` may supervise backend and mobile Ralph-style workstreams through this same coordination directory. It is coordination authority, not repository or security authority: existing ownership, approval, privacy, CI, and frozen-worktree rules remain binding. Explicit owner campaign priority overrides use paused-supervisor `defer-issue` / `resume-issue` scheduling metadata; deferral never edits, closes, verifies, or deletes the canonical issue, and already-assigned work may be released only by explicit exact `--clear-task-id` values rather than fuzzy task/prompt inference. Claude remains the preferred implementer with Codex as implementation fallback. Ralph may use a long-lived Claude Code subscription token only when the operator has explicitly configured it through the paused-supervisor `configure-claude-oauth` command; the token must live in the OS-backed Rex credential vault, config may retain only a credential reference, and the token value may be injected only into the disposable Claude Docker child environment (never argv, repository/config/coordination files, prompts, or logs). If no vault-backed token is configured, preserve the existing read-only Claude credential-file path. Read-only reasoning routes through OpenAI API Terra/Sol with matching Codex fallbacks when explicitly enabled. GPT-6 Astra is API-only emergency adjudication after the Sol chain returns a valid unresolved result, at most once per revision-bound escalation episode; it is never routine planning/review, implementation, or final verification. See `docs/claude/DEVELOPMENT_ORCHESTRATOR.md`.

Control-plane JSON writes must remain atomic. On Windows, if bounded os.replace retries are exhausted by sharing/access failures, use the reviewed native ReplaceFileW fallback for an existing destination; do not work around a control-plane write failure with ad-hoc in-place JSON edits.

Live-test review task IDs may be descriptive wrappers around a canonical TEST-### token; issue-update safety must match the exact bounded token, still verify ownership/existence/status, and treat an already fixed-needs-retest transition as idempotent rather than forcing a false mismatch. The orchestrator OpenAI API worker is read-only reasoning infrastructure. `scripts/dev_orchestrator/openai_worker.py` may review/plan/adjudicate only through bounded evidence, reservation-before-send API budgeting, canonical result validation, exact invocation binding, and same-HEAD durable handoff. `scripts/dev_orchestrator/scratch.py` owns the shared disposable-clone helpers used by model workers. Live API routing is disabled by default and requires a dedicated OpenAI project with an operator-confirmed enforced $30.00/month provider limit, a vault-backed household OpenAI credential, and the separate `enable-openai-worker` command. Normal orchestrator `activate` never enables paid API routing. Ralph also enforces its own local $30.00-per-UTC-month reservation ledger. API billing/rate limits never consume ChatGPT/Codex banked reset accounting, and `FINAL-VERIFY-*` always bypasses the API worker for Codex GPT-5.6 Sol.

Never activate orchestrator CLI writers while browser-based coding workers are still writing the same worktrees. Rollout starts observe-only and uses a trusted-local-operator handoff: the supervisor issues a nonce request bound to a browser-session correlation ID; the Windows operator verifies that worker checkpointed a clean linked task worktree and stopped writing, then records `operator-ack-handoff`. Lease acceptance waits through a quiescence interval and revalidates repository identity, exact `origin/<current-branch>` upstream, branch, HEAD, and cleanliness. Supervisor and per-role control-plane lock pathnames persist; ownership is an OS/kernel-held exclusive file lock and is never reclaimed by PID inspection or pathname deletion. Supervisor heartbeat health requires the exact heartbeat object, a strictly typed positive integer PID, a timezone-bearing timestamp within bounded future skew, and the exact raw Windows process-creation FILETIME matching the live PID; malformed, future-dated, or PID-reused heartbeats are stale. The browser-session ID is correlation evidence, not cryptographic worker authentication; the trusted Windows operator account is the control-plane authority. Any later external mutation invalidates the lease/provenance gate. The supervisor must reject protected branches, detached or primary checkouts, wrong repository identity, dirty state, and commits that lack the current invocation provenance trailer. Every production model invocation is isolated from the leased worktree. Claude implementation runs in the pinned AskRex Docker sandbox as a non-root user with dropped Linux capabilities and no Bash tool, with only a disposable scratch clone mounted writable. Production model prompts must be delivered over stdin rather than Windows process argv so coordination/review/validation context cannot exceed CreateProcess command-line limits. Claude is the preferred implementer; a classified Claude usage/session limit immediately falls back to Codex implementation in a writable disposable scratch clone under the same lease, provenance, result-binding, and checkpoint gates. Claude-only tool restrictions must not be copied into the Codex fallback prompt: Codex may use workspace-local repository and terminal tools inside its disposable scratch clone, but still may not use network/Web/MCP access or create commits because the supervisor owns authoritative validation and checkpointing. Codex review/lead invocations remain read-only with cwd and `-C` rewritten to disposable scratch clones; OpenAI API reasoning is tool-free and same-HEAD read-only, and any checkpoint implemented by Codex must receive a fresh GPT-5.6 Sol review. Direct Codex review must consume the same revision-bound bounded review evidence as API review, including current contents of changed files when a small incremental diff lacks enough surrounding context. Direct review is bounded-evidence-only: do not invoke terminal/shell/repository/Web/MCP tools. If a material fact is absent, request that specific evidence through changes_required instead of reconstructing it through a repository terminal. Every published `ready_for_review` implementation checkpoint must first pass deterministic local validation gates selected from trusted orchestrator configuration; a normal gate failure returns the exact gate/command/exit/output to implementation feedback, while validation-infrastructure failure parks `blocked_system` and preserves the durable implementation result so recovery retries validation without reinvoking the worker. If both Claude and Codex are usage-limited, the workstream pauses through the existing usage-limit blocker. The leased live worktree, peer repo, shared coordination root, and frozen acceptance worktree are not model working directories. All agent output must pass the canonical Draft 2020-12 JSON Schema before any state change, coordination write, or repository publication. Agent outcomes are also phase-bound before any side effect: implementation may return only continue/ready_for_review or a blocker/failure; review only pass/changes_required or a blocker/failure; planning/adjudication only assign/done or a blocker/failure. An outcome valid for another phase must fail closed before coordination publication. Successful Codex review/lead and OpenAI reasoning output is schema-validated before lease advancement, and every planning/adjudication result passes the same supervisor coordination-publication safety gate before its outcome can change workflow state. Every production Claude result, including blocked or failed outcomes that carry coordination messages, must echo the exact active role, task ID, and unpredictable invocation ID before it can cause any live side effect. Production Codex invocations must additionally use an invocation-bound output schema whose `role`, active `task_id` for implement/review, and unpredictable `invocation_id` are pinned to supervisor-generated single-value enums; do not rely on model transcription alone for these authority bindings. For successful implementation outcomes only, the supervisor creates the invocation-provenance commit inside the scratch clone, revalidates the exact clean leased HEAD, imports that immutable direct-child commit, and fast-forwards the live worktree while holding the per-role lease lock. Custom executors are test-only, require every configured runtime path to live beneath the system temporary root, receive only scratch clones, must leave their configured repository unchanged, and never publish or advance leased repositories. Custom executors are trusted test fixtures only, require explicit test opt-in and temporary runtime paths, and are not a security boundary for hostile callback code. Repository-state seals and child-process cleanup provide defense-in-depth against accidental ambient mutation, including hidden refs, scratch-origin pushes, modify-then-restore writes, exception paths, and Windows reparse traversal; untrusted or adversarial callbacks must never be supplied through this hook. Docker image/container inspection and cleanup distinguish present/absent/unknown state; images are built only when definitively absent, and daemon/inspection ambiguity fails closed. Coordination mailbox publication is a recoverable transaction: a durable manifest is written before visibility, all destinations are mailbox-confined, partial writes roll back, abandoned manifests are recovered under the publication lock, and supervisor startup performs recovery before normal work. A successful Claude child keeps its activity marker through a fail-closed `postprocessing` phase until checkpoint publication and lease advancement are durable; the watchdog must preserve postprocessing or dirty unpublished scratch work rather than delete it, and any in-process publication/checkpoint failure before durable lease advancement must retain that scratch clone and marker for recovery. Each production scratch has a random ownership nonce recorded both in its ownership manifest and activity marker; watchdog cleanup verifies nonce, canonical path, role, invocation ID, recorded pre-HEAD, Git top-level/HEAD, and rejects the root or any descendant reparse point, captures a stable Windows filesystem identity immediately before quarantine, atomically renames the verified scratch parent into a private quarantine path, requires the same filesystem identity after rename, revalidates provenance and reparse safety there, and deletes it with a non-reparse-following primitive; ambiguous rename/revalidation/deletion fails closed and retains quarantine. Banked Work/Codex reset consumption, physical-device testing, credentials/account actions, and authority-changing operations remain human-confirmed gates; only the matching confirmation/resume command may release a blocked workstream. Claude CLI `session limit` or monthly-spend-limit responses (including HTTP 429 API-error JSON envelopes that may exit with process code 0) are usage-limit signals, not implementation failures: implementation must first fall back to Codex, and only a resulting provider usage-limit condition that leaves no implementation provider available may park the workstream through the existing reset-confirmation path. A cross-role `dependency` pause may release automatically only after the peer has moved off the same task family and the dependent role's mailbox contains an explicit peer-authored `Needs response: no` contract message for that family; merely seeing a coordination message while the peer is still working is insufficient. A schema-validated, identity-bound agent result must be journaled atomically with lease advancement before an activity marker is cleared; on restart, the supervisor consumes that durable result before any reinvocation and records the applied invocation ID in worker state so crash replay is idempotent. When an accepted result immediately triggers a new model phase, such as review rejection entering adjudication, the supervisor must first persist/apply the current result and clear its pending journal receipt before invoking the next phase; never begin a new phase while the prior phase result is still pending. Reviewed task completion is durable supervisor state, not an inference from mailbox freshness. Record a completed-task ledger entry only after independent review passes or a valid coordination-only adjudication closes the task. Planning context must include applicable completed work, and the supervisor must reject a planner assignment that maps to reviewed completed work while that reviewed checkpoint remains an ancestor of the current branch; stale Needs-response mailbox history may not resurrect it. After independent review passes and a task clears to idle, treat idle as a transient scheduling state and invoke planning again on the next cycle so an active campaign continues without external nudges. Small Python-only backend implementation tasks may use the trusted scoped-diff validation profile when enabled: the current task diff must be bounded, contain changed focused regression tests, and contain only Python files; validate those changed tests, Ruff all changed Python files, compile changed source files, and diff-check the task revision. Any non-Python, broad, ambiguous change or task without focused changed regressions falls back to explicit task gates or the normal broad suite. Coordination-only closeouts are not product-code work: unless a trusted task-specific validation override exists, validate them with the coordination recheck plus unchanged-checkpoint/diff checks rather than the default full backend/mobile suite. A current revision-bound passing validation receipt is authoritative for the deterministic gates configured for that task; reviewers may not re-litigate superseded gate failures or invent unrelated required gates. Historical review feedback is context only after a fresh passing validation receipt. After two review rejections on an explicit coordination-only task with a current green receipt, escalate to Astra adjudication instead of cycling implementation indefinitely; Astra may close only that coordination-only task when requirements are already satisfied, never waive product-code/security/authority defects. A fixed-needs-retest handoff belongs to testing and must clear the backend/mobile development task back to idle so the active campaign continues while physical verification remains pending. Explicit queued work is checked before planning and must always win. A role may park only for a truthful blocker, observe-only mode, or a verified done transition through the deterministic completion/final-review gates; ordinary task completion is never itself a reason to stop the loop.
Every transient Codex Windows terminal-runner failure path, including an AgentInvocationError raised before a model result exists, must invoke the canonical runner recovery before incrementing retry state. Keep regression coverage for both blocked-result and invocation-exception paths so retries cannot exhaust while the stale sandbox service remains uncleared.
- On Windows Codex implementation scratch clones, do not remove the .git entry entirely while hiding Git metadata: Codex 0.156.x can then fail terminal creation with the runner pipe-in timeout. Move the real .git directory out of the workspace, leave a gitfile pointing to an intentionally empty non-repository placeholder directory so Git commands fail closed, and restore the real metadata before supervisor checkpointing. Keep a regression proving the marker is present, Git operations remain unavailable to the model, and the real metadata is restored afterward.

Review evidence must include task-matching canonical live-test issue state in a dedicated bounded section rather than relying on its position inside generic coordination context. Review evidence must not discard an otherwise complete revision-bound bundle merely because supplemental current changed-file snapshots exceed their section budget. When the authoritative task diff is complete and the matching validation receipt is present, bounded/truncated changed_file_contents is noncritical supplemental context; preserve the rich review packet rather than falling back to the thin coordination snapshot.
