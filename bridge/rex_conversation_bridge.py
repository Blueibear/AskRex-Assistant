...
def process_payload(payload: dict[str, object], *, store: HistoryStore | None = None) -> dict[str, object]:
    """Run one private, owner-scoped canonical conversation operation."""
    ...
    store = store or HistoryStore(household_data_path("history.db"))
    if action == "create":
        ...
        result = {
            ...
            "messages": store.load_history(user_id, conversation_id=conversation_id),
        }
    elif action == "rename":
        result = store.rename_conversation(
            user_id, str(payload["conversation_id"]), str(payload.get("title") or "")
        )
    elif action == "archive":
