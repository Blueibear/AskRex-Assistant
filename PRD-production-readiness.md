...
**Backend bounded review packet 2026-09-26 (source-verified re-check, no code changes):** independent review reported the supplied bounded evidence omitted the canonical story, a matching validation receipt, diff/changed-file evidence, focused test results, the mobile contract publication, an issue update, and the coordination checks. This worker is Bash/shell-less in this environment (deterministic supervisor owns test execution, `git diff`, and `scripts\check-coordination.ps1`), so this entry supplies only what is independently source-verifiable from the current backend worktree and explicitly flags what remains supervisor-owned:

- Canonical story/acceptance criteria: this file, `PRD-production-readiness.md:3067-3145` (`US-086`), decomposition directive at `PRD-production-readiness.md:3091` scoping this work to slice (a) plus the contract-publication follow-up above.
- Canonical wire contract: `docs/mobile/MOBILE_API_MASTER_SPEC.md:412-445` (§6.5) for `POST /mobile/attachments` request/limits/response shape and the `attachment_ids`/`attachments` chat fields at §6.3; `rex/mobile_api/capabilities.py:83` reports `"attachments": True` unconditionally; `tests/mobile_api/contract_vectors.json:2,109-112,170` pins `contract_version 2026-09-25.323.7` with `attachments` under both the chat and capabilities fixtures.
- Changed/implementing files re-read and confirmed present and internally consistent with the above contract in this pass (no further edits required): `rex/mobile_api/attachments.py` (sniffing/store, `_cleanup_expired` retry-safe delete at lines 289-307, insert/commit rollback cleanup at lines 372-385), `rex/mobile_api/routes/attachments.py` (scope/session revalidation, bounded incremental read, exact two-part multipart enforcement), `rex/mobile_api/routes/chat.py:126,219` (`attachment_store.validate_references` binds `attachment_ids` to owner/device/conversation before use), `rex/mobile_api/db.py:162-177` (`mobile_conversation_attachments` schema/index).
- Focused backend test file confirmed present with named coverage matching the acceptance criteria: `tests/mobile_api/test_attachments.py` (`TestMobileAttachments`, 19 test methods covering scope enforcement, repeated/oversized/lengthless multipart, structural PNG/JPEG/PDF validation and forged-signature rejection, owner/device/conversation binding over HTTP/WebSocket/SSE, count+expiry cleanup, insert-failure and unlink-failure retry safety) and `tests/mobile_api/test_status_capabilities.py` (`attachments` capability stays `True` independent of chat/STT/TTS availability).
- Not independently claimed here: actual pytest pass/fail counts, `ruff`/lint output, and `git diff --check origin/master...HEAD` — this worker has no Bash/shell tool in this session. Requested commands for the deterministic supervisor: `pytest -q tests/mobile_api/test_attachments.py tests/mobile_api/test_status_capabilities.py tests/mobile_api/test_contract_vectors.py`, `git diff --check origin/master...HEAD`.
- Mobile contract publication and both opening/final `scripts\check-coordination.ps1 -Role backend` coordination-check outputs live in the shared coordination root, which this worker cannot write to directly; the exact contract text above has been resent via coordination message this pass and the coordination-check runs are requested from the supervisor/orchestrator.
- No source or test changes were made in this pass; the prior 2026-09-25 implementation and contract-publication fixes remain the full accepted diff for this slice.

**Backend bounded review packet 2026-09-26 (second re-check, no code changes):** independent review again requested a resubmitted bounded package covering canonical story, diff/changed-file evidence, focused test results, mobile contract publication, canonical issue update, and coordination checks. This worker remains Bash/shell-less this session (deterministic supervisor owns `pytest`, `git diff`, and `scripts\check-coordination.ps1`); the following is exactly what is independently source-verifiable from the current backend worktree:

- Canonical story: this file, `PRD-production-readiness.md:3067-3145` (`US-086` acceptance criteria), scoped to decomposition slice (a) at line 3091 plus the contract-publication follow-up at lines 3138-3144.
- Re-read and confirmed unchanged/consistent in this pass: `rex/mobile_api/attachments.py` (structural PNG/JPEG/PDF/HEIC sniffing at lines 58-267, retry-safe expiry cleanup at lines 289-307, insert/commit-failure orphan cleanup at lines 372-385), `rex/mobile_api/routes/attachments.py` (scope enforcement, incremental bounded read at lines 24-33, exact-two-part multipart enforcement at lines 63-73, principal revalidation at lines 83-87), `docs/mobile/MOBILE_API_MASTER_SPEC.md:412-445` (§6.5 request/limits/response shape, matching the code exactly), `rex/mobile_api/capabilities.py:83` (`"attachments": True` unconditional), `tests/mobile_api/contract_vectors.json:2,111-132,170` (`contract_version 2026-09-25.323.7`, `attachments` upload/response fixture, `capabilities_response.features.attachments: true`).
- Focused backend test file `tests/mobile_api/test_attachments.py` re-read in full this pass; confirmed present test methods: `test_chat_attachment_reference_count_overflow_is_payload_too_large`, `test_websocket_attachment_reference_count_overflow_is_payload_too_large`, `test_lengthless_oversized_multipart_is_rejected_during_route_parsing`, `test_lengthless_upload_stream_is_read_in_bounded_chunks`, `test_requires_attachment_scope_before_file_processing`, `test_rejects_repeated_conversation_id_field`, `test_ingests_private_attachment_and_returns_safe_provenance`, `test_sanitized_filename_never_retains_client_directories`, `test_declared_mime_and_extension_do_not_grant_type_authority`, `test_image_content_requires_a_structurally_valid_container`, `test_image_signature_without_complete_structure_is_rejected`, `test_pdf_with_startxref_pointing_outside_the_file_is_rejected`, `test_attachment_reference_is_bound_to_owner_device_and_conversation`, `test_websocket_attachment_reference_is_bound_to_owner_device_and_conversation`, `test_sse_terminal_includes_safe_attachment_provenance`, `test_websocket_terminal_includes_safe_attachment_provenance`, `test_attachment_count_and_temp_expiry_cleanup`, `test_metadata_persistence_failure_removes_created_attachment_file`, `test_expired_file_unlink_failure_keeps_retryable_metadata`, `test_stuck_expired_row_does_not_block_active_quota` (plus `tests/mobile_api/test_status_capabilities.py` covering the unconditional `attachments` capability).
- Not independently claimed here (requires Bash, unavailable this session): actual `pytest` pass/fail counts, ruff/lint output, `git diff --check origin/master...HEAD` exit status. Requested from the deterministic supervisor: `pytest -q tests/mobile_api/test_attachments.py tests/mobile_api/test_status_capabilities.py tests/mobile_api/test_contract_vectors.py`, `git diff --check origin/master...HEAD`, and `scripts\check-coordination.ps1 -Role backend` (opening and final).
- Mobile wire-contract publication (exact §6.5 request/response shape above) and a testing retest request are resent this pass via structured coordination messages, since this worker has no filesystem write access to the shared coordination root.
- No source or test changes were made in this pass; slices (b)-(f) remain out of scope and not started.

**Backend bounded review packet 2026-09-26 (third re-check, no code changes — closing the publication/gate gap):** the prior two packets above were source-verification only and did not actually transmit a `to: mobile` mailbox message, so the "cross-repository publication" gap review kept citing was real even though the documented contract in `docs/mobile/MOBILE_API_MASTER_SPEC.md` was correct. This pass:

- Sends an explicit `backend -> mobile` coordination message (not `backend -> backend`) restating the exact `POST /mobile/attachments` request/response shape from §6.5 verbatim, with `needs_response: true`, so mobile has a canonical notification it can act on before depending on the `attachments` capability/contract.
- Sends an explicit `backend -> supervisor` coordination message requesting the deterministic supervisor run the specific missing gate commands and report their outcome, following the same pattern that already unblocked the unrelated S35 VoiceStudio review (`MSG-zzzz-s35-four-outcome-supervisor.md`): `pytest -q tests/mobile_api/test_attachments.py tests/mobile_api/test_status_capabilities.py tests/mobile_api/test_contract_vectors.py`, `git diff --check origin/master...HEAD`, and `scripts\check-coordination.ps1 -Role backend` (opening and final). This worker has no Bash/shell tool in this session and will not fabricate pass/fail counts, diff output, or coordination-check output.
- Re-confirms via direct source read in this pass (no edits needed): `rex/mobile_api/attachments.py` (structural sniffing lines 58-267; retry-safe expiry cleanup lines 289-307; insert/commit-failure orphan cleanup lines 372-385), `rex/mobile_api/routes/attachments.py` (auth/scope, bounded incremental read, exact two-part multipart enforcement, principal revalidation), `docs/mobile/MOBILE_API_MASTER_SPEC.md:412-445` (§6.5, matches code), `rex/mobile_api/capabilities.py:83` (`"attachments": True` unconditional), `tests/mobile_api/contract_vectors.json` (`contract_version 2026-09-25.323.7`, `attachments` fixtures present), `tests/mobile_api/test_attachments.py` (19 named test methods, matching acceptance criteria).
- No new implementation or test gap was identified in this pass; slices (b)-(f) remain out of scope and not started. Once the supervisor-run gate/coordination-check output above is available, it can be appended here verbatim without requiring further source changes.

**Backend closeout 2026-09-26 (real gate execution, two fixes, cloud worker with Bash access):** every prior packet above was source-inspection only because that worker had no shell tool. This pass ran on the same HEAD (`a3b4f13b`) with a real Bash/pytest environment and found two genuine defects the source-only re-checks could not have caught, both now fixed in this commit:

- `tests/mobile_api/test_config.py::TestMobileApiConfigSerialization::test_serialized_group_is_json_safe_and_secret_free` asserted an exact `mobile_api` config key set that predated the `max_attachment_bytes`/`max_attachments_per_conversation`/`attachment_retention_seconds` fields added for this slice; it failed on a clean run. Fixed by adding the three fields to the expected set (they are non-secret bounded integers, consistent with the test's actual purpose).
- `rex/mobile_api/routes/attachments.py` and `tests/mobile_api/test_attachments.py` violated Black under the pinned formatter revision (`26.3.1`, matching `.pre-commit-config.yaml`; the ambient `pip install black` resolves to a newer, differently-behaving `26.5.1`). Reformatted with the pinned version per CLAUDE.md's rule against unpinned formatters in a required check.

Actual executed evidence (not claimed from source reading): `pytest -q tests/mobile_api/` → 404 passed; `pytest -q tests/mobile_api/test_attachments.py tests/mobile_api/test_status_capabilities.py tests/mobile_api/test_contract_vectors.py` → 59 passed; `pytest -q tests/test_us074_document_indexing.py` → 12 passed; `ruff check` on the full changed/relevant file set → all checks passed; `black --check` (pinned `26.3.1`) → clean after the two reformats above; `python scripts/check_skip_inventory.py` → OK, 129 sites current. This corroborates, and does not conflict with, the independently-run Windows gate receipt already recorded at this same `a3b4f13b` head (`state/validation/backend-roadmap-us086-mobile-attachments.json`: 138 passed across the broader attachment/chat/grant/strong-auth regression set, `ruff check` clean, `py_compile` clean, `git diff --check` clean against `db012bf2`), which did not separately run Black and therefore did not surface the formatting gap.
