...
Implementation may return `continue` or `ready_for_review`. A published `ready_for_review` checkpoint must pass the supervisor's configured deterministic local validation gates before the workstream may enter independent review. Explicit coordination-only closeouts use a lightweight default profile (coordination recheck, unchanged checkpoint, and diff check) unless trusted configuration supplies a stronger task-specific override; they do not inherit the full product-code test suite merely because they run on a backend/mobile role. The current matching revision-bound receipt defines the deterministic gates the reviewer may require, so superseded failures and unrelated unconfigured gates are not valid reasons to reopen implementation. Successful validation clears prior implementation/reviewer feedback from the authoritative review input. Two review rejections on a coordination-only task with a current green receipt trigger Astra adjudication instead of another ordinary implementation loop; Astra may close only such no-code coordination work and cannot waive source, security, permission, or authority defects. A fixed-needs-retest issue update hands the remaining physical acceptance to testing and returns the development role to idle/planning rather than blocking that role until the retest occurs. Normal gate failures return the exact gate, command, exit code, and captured output to implementation feedback; validation-infrastructure failures park as `blocked_system` while preserving the durable implementation result so recovery retries validation without reinvoking the worker. Independent review must return `pass` before the task is cleared. Current contents of changed files are included for small diffs so reviewers can verify surrounding pre-existing regression/source context without reconstructing it through a shell. Direct Codex review consumes the same revision-bound bounded evidence and is tool-free: it must not invoke terminal/shell/repository/Web/MCP tools; when a material fact is absent it returns changes_required naming that evidence instead. Reviewed completions are persisted in the supervisor completed-task ledger. The planning lead receives that bounded history, and a planner assignment that maps to a reviewed completion is rejected before implementation while the reviewed checkpoint remains an ancestor of the current branch. Stale mailbox Needs-response entries therefore cannot resurrect already-reviewed work; a genuine regression must be represented by new explicit work. After a reviewed task clears to idle, idle is only a transient scheduling state: on the next cycle the supervisor plans the next bounded task even when repository and coordination context are otherwise unchanged. Explicit queued work is checked first and always wins. A role parks only for a truthful blocker, observe-only mode, or a verified done transition through deterministic completion and final-review gates; completing one ordinary task never stops an active campaign. `changes_required` returns review feedback to the implementer. Repeated failures escalate within the configured routing policy. Phase escalation is journal-safe: the supervisor records/applies the triggering result and clears its durable pending receipt before invoking adjudication or any other next model phase. Astra is considered only for adjudication after the API Sol and Codex Sol chain returns a valid explicitly unresolved `failed` result and the emergency threshold/episode/budget gates pass.
...
py -3.11 -m scripts.dev_orchestrator.cli configure-claude-oauth --coordination-root C:\Users\james\rex-ai-test\askrex-coordination
...
Claude Code usage-limit detection also covers API-error JSON envelopes returned with process exit code 0, including HTTP 429 monthly-spend-limit responses; these enter the existing Codex implementation fallback rather than being counted as malformed agent output.

Live-test tasks may use descriptive task IDs containing an exact canonical TEST-### token. Review issue-update safety accepts only that exact bounded token, still verifies the issue exists and is owned by the active role, and treats an already fixed-needs-retest state as idempotent. Reviewers must not emit a redundant retest transition when the authoritative issue evidence already shows fixed-needs-retest.
