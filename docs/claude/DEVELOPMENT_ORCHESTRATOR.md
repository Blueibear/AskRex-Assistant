...
`init` is non-destructive and defaults to observe-only. `cycle` performs one supervision cycle. `run` holds the single-instance lock, maintains a background heartbeat, reloads config every cycle, and continues until stopped. Normal `activate` changes only observe-only/runtime state; it does **not** enable paid OpenAI API routing.
...
Planner/adjudicator `done` claims are advisory only, and Astra may not mark an active task done before independent review. Each repository has an explicit completion manifest. Before a workstream becomes `done`, the deterministic gate requires no owned open or `fixed-needs-retest` issues, a clean leased worktree, revision-bound documentation-truth acceptance, recorded physical acceptance, recorded cross-repo contract acknowledgment, a CI workflow, an OPEN pull request at the exact current HEAD and expected base branch, every mandatory named CI check present with conclusion SUCCESS, and every local release-quality command passing. A final independent **Codex GPT-5.6 Sol** review runs only after those deterministic gates pass and always bypasses the OpenAI API worker, even when paid routing is enabled and budget remains. Missing GitHub authentication is a `blocked_user` credential boundary, not a coding task. `SKIPPED` or `NEUTRAL` never satisfies a required check.
