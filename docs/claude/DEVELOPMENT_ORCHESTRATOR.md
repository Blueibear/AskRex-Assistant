...
Implementation may return `continue` or `ready_for_review`. A published `ready_for_review` checkpoint must pass the supervisor's configured deterministic local validation gates before the workstream may enter independent review. Normal gate failures return the exact gate, command, exit code, and captured output to implementation feedback; validation-infrastructure failures park as `blocked_system` while preserving the durable implementation result so recovery retries validation without reinvoking the worker. Independent review must return `pass` before the task is cleared. After a reviewed task clears to idle, the supervisor records a content-free fingerprint of that role's repository HEAD plus bounded coordination context; unchanged idle context must not trigger another planning invocation, while a repo, issue, or mailbox change automatically wakes planning. Explicit queued work is checked before this idle watermark and always wakes the role. This idle watermark is scheduling memory only and never substitutes for review, testing, or completion verification. `changes_required` returns review feedback to the implementer. Repeated failures escalate within the configured routing policy. Astra is considered only for adjudication after the API Sol and Codex Sol chain returns a valid explicitly unresolved `failed` result and the emergency threshold/episode/budget gates pass.
